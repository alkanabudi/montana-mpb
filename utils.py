...
def get_data_mpb_2025():
    try:
        ...
        if col_nilai in df.columns:
            # Membersihkan simbol Rp, titik, dan koma
        ...
        return df # SUDAH DIPERBAIKI (Hanya return df)

    except Exception as e:
        st.error(f"Gagal koneksi ke Sheet MPB 2025: {e}")
        return pd.DataFrame() # SUDAH DIPERBAIKI (Hanya return df)